...
// Reviza@Changes 09-04-26 --- Inject buffer into unifiedResponse.data to support proper rendering of rich messages (ex: tables and code blocks)
...
export const prepareRichResponseMessage = (content) => {
    const { alignment, code, contentText, disclaimerText, fakeVerification, footerText, headerText, imageText, inlineImage, inlineVideo, items, language, latex, links, messageSecret, noHeading, posts, products, suggested, richResponse, table, tapLinkUrl, title } = content;
    let submessages = [];
    ...
    const richResponseMessage = proto.AIRichResponseMessage.create({
        ...
        unifiedResponse: {
            data: Buffer.from(JSON.stringify(unified)) // Reviza@Note 25-04-26 --- Expects "ArrayBufferLike"
        },
        contextInfo: {
            isForwarded: true,
            forwardingScore: 1,
    ...
    const message = wrapToBotForwardedMessage(richResponseMessage, {
        fakeVerification: content.fakeVerification,
        messageSecret: content.messageSecret
    });
    const botMetadata = message.messageContextInfo.botMetadata;
    // Reviza@Note 13-05-26 --- Add disclaimer text on richResponseMessage
    if (disclaimerText) {
        botMetadata.messageDisclaimerText = disclaimerText;
    }
    // Reviza@Note 15-05-26 --- Add responseId from unified directly to botMetadata
    botMetadata.botResponseId = uuid;
    return message;
};
// Reviza@Note 17-04-26 --- signature and certificateChain for proofs[] field
...
/**
 * Reviza@Changes 14-09-26 --- Tambah messageSecret agar pesan AI Rich langsung tampil
 * tanpa peringatan "WhatsApp tidak dapat memastikan pesan ini aman" + tombol Unduh.
 *
 * Sebelumnya verificationMetadata selalu diisi sertifikat & tanda tangan acak. WhatsApp
 * mencoba memvalidasinya, gagal, lalu menahan konten. Kini field itu tidak dikirim
 * kecuali diminta lewat opsi `fakeVerification: true`.
 *
 * @param {object} richResponseMessage isi AIRichResponseMessage
 * @param {object} [options]
 * @param {boolean} [options.fakeVerification=false] kirim verificationMetadata acak (perilaku lama)
 * @param {Buffer}  [options.messageSecret] messageSecret custom, default 32 byte acak
 */
export const wrapToBotForwardedMessage = (richResponseMessage, options = {}) => {
    const { fakeVerification = false, messageSecret } = options;
    const botMetadata = {};
    if (fakeVerification) {
    ...
    return {
        messageContextInfo: {
            // Reviza@Note 14-09-26 --- dipakai WhatsApp untuk integritas pesan bot
            messageSecret: messageSecret || randomBytes(32),
            botMetadata
        },
        botForwardedMessage: {
