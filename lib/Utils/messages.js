...
// Reviza@Changes 31-01-26 --- Extract product message into a standalone function so it can also be reused as the header for interactive messages
const prepareProductMessage = async (message, options) => {
    ...
    // Reviza@Changes 01-02-26 --- Add product message default value
...
/**
 * Reviza@Note 30-01-26
 * ---
 * Credits: Work on ensuring stickerPackMessage fields are valid by @jlucaso1 (https://github.com/jlucaso1).
 * based on https://github.com/WhiskeySockets/Reviza Baileys/pull/1561
 */
// Reviza@Changes 21-04-26 --- Enhanced prepareStickerPackMessage
const prepareStickerPackMessage = async (message, options) => {
    const { cover, stickers = [], name = '📦 Sticker Pack', publisher = 'GitHub: reviza', description = '🏷️ @revizahoshii/baileys' } = message;
    ...
    const logger = options.logger;
    // Reviza@Changes 01-02-26 --- Add caching for sticker pack
...
// Reviza@Changes 30-01-26 --- Add native flow button helper for interactive message
const prepareNativeFlowButtons = (message) => {
    ...
    // Reviza@Changes 31-01-26 --- Add offer and options inside interactive message
    if (hasOptionalProperty(message, 'offerText') && !!message.offerText) {
        Object.assign(messageParamsJson, {
            limited_time_offer: {
                ...
                url: message.offerUrl || DONATE_URL, // Reviza@Note 02-02-26 --- Apologies if this feels cheeky, just a fallback
    ...
    return {
        buttons: correctedField.map(button => {
            ...
            // Reviza@Changes 12-03-26 --- Add "single_select" shortcut ＼⁠(⁠°⁠o⁠°⁠)⁠／
...
// Reviza@Changes 06-02-26 --- Validate album message media to avoid bug 👀
...
// Reviza@Changes 30-01-26 --- Validate carousel cards header to avoid bug 👀
...
export const generateWAMessageContent = async (message, options) => {
    var _a, _b;
    let m = {};
    // Reviza@Changes 30-01-26 --- Add "raw" boolean to send raw messages directly via generateWAMessage()
    if (hasNonNullishProperty(message, 'raw')) {
        delete message.raw;
        return message;
    }
    // Reviza@Changes 09-04-26 --- Add support for code block and table with richResponseMessage
        ...
        if (message.poll.toAnnouncementGroup) {
            ...
            // Reviza@Changes 08-02-26 --- Add quiz message support
            if (message.poll.pollType === 1) {
                ...
                m.pollCreationMessageV5 = {
                    // Reviza@Note 08-02-26 --- quiz for newsletter only
                    ...pollCreationMessage,
                    correctAnswer: {
    ...
    // Reviza@Changes 08-02-26 --- Add poll result snapshot message
    ...
    // Reviza@Changes 08-02-26 --- Add poll update message
    ...
    // Reviza@Changes 01-02-26 --- Add payment invite message
    ...
    // Reviza@Changes 01-02-26 --- Add order message
    ...
    // Reviza@Changes 31-01-26 --- Add support for album messages
    ...
    // Reviza@Changes 30-01-26 --- Add interactive messages (buttonsMessage, listMessage, interactiveMessage, templateMessage, and carouselMessage)
    if (hasNonNullishProperty(message, 'buttons')) {
        const buttonsMessage = {
            buttons: message.buttons.map(button => {
                // Reviza@Changes 12-03-26 --- Add "single_select" shortcut!
                const buttonText = button.text || button.buttonText;
    ...
    // Reviza@Note 03-02-26 --- This message type is shown on WhatsApp Web/Desktop and iOS (I guess ｡⁠◕⁠‿⁠◕⁠｡). On Android, it only appears in newsletter (so far ಥ⁠‿⁠ಥ)
        ...
        hydratedTemplate.templateId = message.id || 'template-' + Date.now(); // Reviza@Note 04-02-26 --- Minimal templateId to satisfy WhatsApp (⁠ ⁠ꈍ⁠ᴗ⁠ꈍ⁠)
        m = {
            templateMessage: {
    ...
    // Reviza@Changes 01-02-26 --- Add request payment message
    ...
    // Reviza@Changes 01-02-26 --- Add invoice message
    ...
    // Reviza@Changes 31-01-26 --- Add direct externalAdReply access (no need to create contextInfo first)
    if (hasOptionalProperty(message, 'externalAdReply') && !!message.externalAdReply) {
        ...
        if (!content.url || typeof content.url !== 'string') {
            content.url = DONATE_URL; // Reviza@Note 02-02-26 --- Apologies if this feels cheeky, just a fallback
        }
    ...
    // Reviza@Changes 31-01-26 --- Add "groupStatus" boolean to set contextInfo.isGroupStatus and wrap message into groupStatusMessageV2
    ...
    // Reviza@Changes 06-05-26 --- Add "spoiler" boolean to set contextInfo.isSpoiler and wrap message into spoilerMessage
    ...
    // Reviza@Changes 02-02-26 --- Add "interactiveAsTemplate" boolean to wrap interactiveMessage into templateMessage
    else if (hasOptionalProperty(message, 'interactiveAsTemplate') && !!message.interactiveAsTemplate) {
        if (!m.interactiveMessage) {
            throw new Boom('Invalid message type for template', { statusCode: 400 }); // Reviza@Note 02-02-26 --- To avoid bug 👀
        }
        m = {
            templateMessage: {
                interactiveMessageTemplate: m.interactiveMessage,
                templateId: message.id || 'template-' + Date.now() // Reviza@Note 04-02-26 --- Minimal templateId to satisfy WhatsApp (⁠ ⁠ꈍ⁠ᴗ⁠ꈍ⁠)
            }
        };
        delete message.interactiveAsTemplate;
    }
    // Reviza@Changes 30-01-26 --- Add "ephemeral" boolean to wrap message into ephemeralMessage like "viewOnce"
    if (hasOptionalProperty(message, 'ephemeral') && !!message.ephemeral) {
        m = { ephemeralMessage: { message: m } };
        delete message.ephemeral;
    }
    // Reviza@Changes 16-05-26 --- Add wrap message into lottieStickerMessage by isLottie boolean
    ...
    // Reviza@Changes 03-02-26 --- Add "viewOnceV2" boolean to wrap message into viewOnceMessageV2 like "viewOnce"
    else if (hasOptionalProperty(message, 'viewOnceV2') && !!message.viewOnceV2) {
        m = { viewOnceMessageV2: { message: m } };
        delete message.viewOnceV2;
    }
    // Reviza@Changes 03-02-26 --- Add "viewOnceV2Extension" boolean to wrap message into viewOnceMessageV2Extension like "viewOnce"
...
export const generateWAMessageFromContent = (jid, message, options) => {
    ...
    // Reviza@Changes 30-01-26 --- Add deviceListMetadata inside messageContextInfo for private chat
...
export const normalizeMessageContent = (content) => {
    ...
    return content;
    // Reviza@Changes 03-02-26 --- Add all futureProofMessage into getFutureProofMessage()
