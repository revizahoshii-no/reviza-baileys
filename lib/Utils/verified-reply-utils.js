 ...
 *
 * Bila `thumbnail` diisi, kutipan memakai `imageMessage` sehingga gambarnya
 * ikut tampil. `contactMessage` sengaja tidak dipakai untuk gambar karena
 * WhatsApp tidak merender foto dari vcard pada pratinjau kutipan.
 *
 * @param {object} [options]
 * @param {string} [options.name] nama/caption yang tampil pada kutipan
 * @param {Buffer} [options.thumbnail] gambar kutipan (mis. foto profil pengguna)
 * @param {string} [options.text] teks kutipan bila tanpa gambar dan tanpa kontak
 * @param {boolean} [options.contact=false] tampilkan sebagai kartu kontak
