 ...
 * sehingga kutipan yang mengaku berasal darinya tampil sebagai
 * "WhatsApp Business (centang biru) - Status".
 */
 ...
 *
 * Avatar bulat pada kutipan diambil WhatsApp dari foto profil nomor yang
 * ditulis pada baris `TEL;waid=` di dalam vcard, bukan dari `participant`.
 * Isi `waid` dengan nomor yang foto profilnya ingin ditampilkan, misalnya
 * nomor pengguna yang menjalankan perintah atau satu nomor tetap milikmu.
 *
 * @param {object} [options]
 * @param {string} [options.name] nama/caption yang tampil pada kutipan
 * @param {string} [options.waid] nomor pemilik foto profil untuk avatar kutipan
 * @param {Buffer} [options.thumbnail] gambar kutipan, memakai `imageMessage`
 * @param {string} [options.text] teks kutipan bila tanpa gambar dan tanpa kontak
    ...
    if (thumbnail) {
        quotedMessage = {
            imageMessage: {
                ...
                fileLength: String(thumbnail.length),
                height: 640,
                width: 640,
            },
        };
...
/**
 * Ambil foto profil sebuah JID sebagai Buffer, untuk dipakai jadi thumbnail kutipan.
 * Mengembalikan null bila pengguna tidak punya foto profil atau gagal diunduh.
 *
 * @param {object} sock instance socket
 * @param {string} jid JID yang diambil foto profilnya
 * @param {number} [timeoutMs=7000] batas waktu unduh
 * @returns {Promise<Buffer|null>}
 */
export const fetchProfileThumbnail = async (sock, jid, timeoutMs = 7000) => {
    try {
