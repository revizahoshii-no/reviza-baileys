 ...
 */
export const fetchLatestRevizaBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/revizahoshii-no/reviza-baileys/main/lib/Defaults/index.js';
    try {
        ...
        if (!response.ok) {
            throw new Boom(`Failed to fetch latest Reviza Baileys version: ${response.statusText}`, { statusCode: response.status });
        }
