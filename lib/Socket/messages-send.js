...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            // Reviza@Changes 02-02-26 --- Normalize message first to extract the original message and valid media type
            ...
            if (isNewsletter) {
                ...
                // Reviza@Changes 08-02-26 --- Add "additionalNodes" for newsletter too (⁠っ⁠˘̩⁠╭⁠╮⁠˘̩⁠)⁠っ
                ...
                binaryNodeContent.push({
                    tag: 'plaintext',
                    attrs: extraAttrs, // Reviza@Changes 02-02-26 --- Add extraAttrs to fix media being rejected when sending to newsletter (⁠◠⁠‿⁠◕⁠)
                    content: bytes
                });
                const stanza = {
                    tag: 'message',
            ...
            // Reviza@Changes 02-02-26 --- Add keepInChat, editedMessage, mediaNotifyMessage and pollUpdateMessage
            const isNeedMetaAttrs = innerMessage?.pinInChatMessage || innerMessage?.keepInChatMessage || innerMessage?.reactionMessage;
            // Reviza@Changes 12-05-26 --- Add groupStatusMessage attributes
            ...
            // Reviza@Changes 02-02-26 --- Add native_flow_name to extraAttrs when sending interactiveResponseMessage
            ...
            // Reviza@Changes 30-01-26 --- Add Biz Binary Node to support button messages
    ...
    const getMediaType = (message) => {
        ...
        // Reviza@Note 02-02-26 --- Add more message type here
    ...
    return {
        ...
        // Reviza@Changes 30-01-26 --- Add support for modifying additionalNodes and additionalAttributes using "options" in sendMessage()
        sendMessage: async (jid, content, options = {}) => {
            const userJid = authState.creds.me.id;
            // Reviza@Changes 13-03-26 --- Add status mentions!
            if (Array.isArray(jid)) {
                ...
                if (isDeleteMsg || isKeepMsg) {
                    ...
                    additionalNodes.push({
                        tag: 'meta',
                        attrs: {
                            // Reviza@Note 08-02-26 --- Still a hypothesis regarding PollResult ༎ຶ⁠‿⁠༎ຶ
                ...
                // Reviza@Changes 30-01-26 --- Add support for AI label in message when "ai" is true, but works only in private chat
                else if (isAiMsg) {
                ...
                // Reviza@Note 06-02-26 --- Refactored to reduce high RSS usage (⁠╥⁠﹏⁠╥⁠)
                if ('album' in content) {
                    const { delayMs = 1500 } = options;
